...
實驗先加 `--benchmark-split development` 選擇方向，再以 `--benchmark-split holdout` 做一次驗證。四個固定 scalar 候選（0.3 / 0.4 / 0.5 / 0.6）維持 `--candidate-strategy-profile scalar_v1`；若 development 確有改善，才可用 opt-in profile 把 class preset 作為獨立候選比較：`scalar_plus_separated_v1` 產生 `preset_separated_v1`，`scalar_plus_separated_hihat_v1` 產生 benchmark-only 的 `preset_separated_hihat_v1`。後者完全繼承 `separated_v1`，只將 closed-hat threshold 從 0.06 降至 0.03；不得套用到 demo 或公開 true-AI preset。兩者都絕不偽裝成 scalar threshold，也不會套用到 scalar 候選。任何 opt-in profile 的 holdout 必須提供同一 manifest 的 development report：`--development-evidence-report /path/outside-repo/performance_benchmark_report.json`；report 需要有 measured improvement，而且該 evidence 只可消費一次。主 scalar benchmark 會明確拒絕單次 preset 的混用，而不是靜默忽略。reference-drums controlled benchmark 也會以相同 preset candidate ID 配對 artifacts，避免把 ADTOF preset 差異誤歸因為 Demucs。即使實驗改善，未通過 fixtures 與可用公開 benchmark 的嚴格 acceptance 前，也不得把 preset 策略升為預設 `recommended_for_practice` 來源。Demo 預設與一般 CI 不受影響。
