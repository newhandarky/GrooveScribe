...
V1 已進入 release candidate finalization 後，tag 前不再新增核心產品功能；只允許 release notes、tag prep、artifact hygiene 或 redaction 文件修正。

`v1.0.0-rc1` 已發布，不能移動或重寫。後續 release candidate 應使用新的遞增 tag；目前建議使用 `v1.0.0-rc2`。
...
本 runbook 只文件化 tag prep，不自動建立 git tag。
