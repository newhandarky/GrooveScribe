...
- V1 預設本機執行，不依賴遠端 inference service。

## 2. Demucs 在系統中的角色

Demucs 的角色是 source separation。它將完整歌曲分離成多個 stems，V1 必要輸出只取 `drums stem` 給下一階段使用。
...
- `drums.wav`
- source separation stage report
...
- `raw_drum.mid`
- drum transcription stage report

設計重點：

- ADTOF-pytorch 不應直接散落在 job manager 或 API 業務流程中。
- 需封裝成 `AdtofDrumTranscriber` adapter。
- adapter 負責模型載入、CLI / Python API 呼叫、錯誤轉換、輸出檔驗證。

## 4. 為什麼 V1 不把 Omnizart 當主線

Omnizart 是多任務音樂轉譜工具，涵蓋 vocal、drum、chord、beat、instrument transcription 等方向。它適合做研究比較或第二模型驗證，但不建議作為 V1 主線，原因如下：

1. 產品目標明確是鼓譜，V1 應使用更聚焦的 drum transcription pipeline。
2. 多任務工具通常安裝依賴、模型管理與錯誤排查成本更高。
3. V1 需要逐段觀察 `drums stem`、`raw MIDI`、`processed MIDI`，Demucs + ADTOF-pytorch 的責任分界更清楚。
4. V1 的核心風險在輸出是否可用，而不是一次整合最多模型。
...
V1 可先不做複雜 audio enhancement，但應保留未來擴充點：
...
此 JSON 是後處理、MusicXML 產生、debug UI 與 regression test 的共同基礎。
...
- Instrument simplification：V1 可將複雜 cymbal 合併成 cymbal，tom 可先簡化成 high / mid / low tom。
...
V1 只需要保留 interface 與 metadata 欄位，不需要實作 fallback。
