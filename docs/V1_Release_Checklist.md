...
- [x] RC pilot handoff bundle 可重跑：`.venv-ai/bin/python scripts/run_v1_rc_pilot.py --output-dir /tmp/groovescribe-v1-rc-pilot`，並以 `.venv-ai/bin/python scripts/check_v1_rc_handoff.py /tmp/groovescribe-v1-rc-pilot/rc_manifest.json` 驗證 redacted manifest / Markdown。
- [x] Final release notes 與 tag prep checklist 已文件化：`docs/V1_Release_Notes.md`、`docs/V1_Tag_Prep_Checklist.md`、`docs/V1_Release_Artifact_Index.md`。
...
- [x] RC pilot manifest 與 handoff Markdown 不含 `/Users/`、`/tmp/`、`/private/tmp/`、`/var/folders/`、`Traceback`、`stdout`、`stderr`、raw command 或 command template。
- [x] Tag prep 文件明確標記 generated release evidence、review packet、RC handoff outputs 不提交。
